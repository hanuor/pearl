...
import com.hanuor.pearl.machineDB.StoreCacheAccessDays;
...
public class ImageLoader {
    ...
    Context ctx;
    StoreCacheAccessDays storeCacheAccessDays;
    ...
    public ImageLoader(Context ctx,RequestQueue mRequestQueue, ImageCache mCache) {
        this.ctx = ctx;
        this.mRequestQueue = mRequestQueue;
        this.mCache = mCache;
        storeCacheAccessDays = new StoreCacheAccessDays(ctx);
    }
    ...
    public ImageContainer get(String requestUrl, ImageListener imageListener,
        ...
        mRequestQueue.add(requestUrl, newRequest);
    ...
    private void evictExistingCache() {
        String date = new SimpleDateFormat("dd").format(new Date());
        if(storeCacheAccessDays.query()== null){
            storeCacheAccessDays.insert(date);
        }else{
            if(Math.abs(Integer.parseInt(storeCacheAccessDays.query()) - Integer.parseInt(date))> 7){
                evictExistingCache();
                storeCacheAccessDays.insert(date);
            }else{
        //do nothing
